...
webix.skin.air = {
	topLayout:"wide",
	//bar in accordion
	barHeight:26,			//!!!Set the same in skin.less!!!
	tabbarHeight: 26,
	rowHeight:26,
	toolbarHeight:22,
...
webix.protoUI({
	...
	show:function(node, mode, point){
		...
		var gridId = $(node).closest("div.webix_dtable[view_id]").attr("view_id");
		var grid = $$(gridId);
		var tag;

		if ($(node).hasClass("webix_inp_static")) {
			tag = $(node).closest("td[column]");
		}

		var colIndex = -1;

		if (tag && tag.prop("tagName") === "TD") {
			colIndex = Number(tag.attr("column"));
		}

		if (!isNaN(colIndex) && colIndex > -1) {
			var colId = grid.columnId(colIndex);
			var colConfig = grid.getColumnConfig(colId);

			if (colConfig.option.filterWidth) {
				fitSize = colConfig.option.filterWidth;
			}
		}

		$container.width(size[0] + fitSize);
		$inner1.width(size[0] + fitSize - 20);
		$inner2.width(size[0] + fitSize - 20);
	},
	_hide:function(e){
...
webix.EditAbility={
	...
	_handle_live_edits:function(keyCode){
		webix.delay(function(){
			var editor = this.getEditor();
			if (editor && editor.config.liveEdit){
				...
				this.callEvent("onLiveEdit", [state, editor, keyCode]);
			}
		}, this);
	},
		...
		*/
		...
		if (editor){
			...
			if (type.config.liveEdit)
				this._live_edits_handler = this.attachEvent("onKeyPress", function(keyCode) {
					this._handle_live_edits(keyCode);
				});
...
webix.editors = {
	...
	"select":{
		...
		render:function(){
			...
			return webix.html.create("div", {
				"class":"webix_dt_editor"
			}, "<select onkeydown='if (event.keyCode==13) {this.blur();}'>"+html+"</select>");
		}
	},
	popup:{
		focus:function(){
...
webix.Date={
	...
	strToDate:function(format,utc){
		...
		return new Function("date","if (!date) return ''; if (typeof date == 'object') return date; if (date.length === 8) {date = date.substring(0, 4); + '-' + date.substring(4, 6) + '=' + date.substring(6, 8);}; var set=[0,0,1,0,0,0]; "+splt+" return new Date("+code+");");
	},
...
webix.extend(webix.ui.datatable,{
	...
	_collectValues:function(id, value,  values, checks){
		...
		values.sort(function(a,b){ return String(a.value) > String(b.value) ? 1 : -1;  });
	}
});
...
webix.protoUI({
	...
	_suggest_config:function(value){
		...
		if (this._settings.optionWidth)
			suggest.width = this._settings.optionWidth + 100;
		else
