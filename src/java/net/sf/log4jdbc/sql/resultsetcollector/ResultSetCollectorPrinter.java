...
public class ResultSetCollectorPrinter {
	...
	public String getResultSetToPrint(ResultSetCollector resultSetCollector) {
		...
		for (int column = 1; column <= columnCount; column++) {
			try {
				maxLength[column - 1] = resultSetCollector.getColumnName(column).getBytes("ksc5601").length;
			} catch (UnsupportedEncodingException e) {
				System.err.println("resultset encoding error");
			}
		...
		if (resultSetCollector.getRows() != null) {
			for (List<Object> printRow : resultSetCollector.getRows()) {
				...
				for (Object v : printRow) {
					if (v != null) {
						try {
							int length = v.toString().getBytes("ksc5601").length;
		...
		if (resultSetCollector.getRows() != null) {
			for (List<Object> printRow : resultSetCollector.getRows()) {
				...
				for (Object v : printRow) {
					try {
						this.table.append(padRight(v == null ? "null" : v.toString(), maxLength[colIndex] - v.toString().getBytes("ksc5601").length + v.toString().length()) + "|");
					} catch (UnsupportedEncodingException e) {
						System.err.println("resultset encoding error");
					}
